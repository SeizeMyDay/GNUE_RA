...
void loop() {
  ...
  for (j=0; j<10; j++) {
    ...
    Distance = analogRead(S1_EchoPin);
  ...
  for (j=0; j<10; j++) {
    ...
    //Duration = analogRead(S2_EchoPin);
    //Distance = ((340 * Duration) / 10000) / 2;
    Distance = analogRead(S2_EchoPin);
  ...
  for (j=0; j<10; j++) {
    ...
    // Duration = digitalRead(S3_EchoPin);
    // Distance = ((340 * Duration) / 10000) / 2;
    Distance = analogRead(S3_EchoPin);
